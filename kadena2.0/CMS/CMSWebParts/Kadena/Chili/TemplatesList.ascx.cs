...
namespace Kadena.CMSWebParts.Kadena.Chili
{
    public partial class TemplatesList : CMSAbstractWebPart
    {
        ...
        private void SetupTemplatesList()
        {
            ...
            {
                repTemplates.DataSource = templatesData.Select(d => new { EditorUrl = string.Format("{0}?id={1}&skuid={2}&templateid={3}&workspaceid={4}", ProductEditorUrl, DocumentContext.CurrentDocument.DocumentID, ECommerceContext.CurrentProduct.SKUID, d.templateId, DocumentContext.CurrentDocument.GetStringValue("ProductChiliWorkgroupID", string.Empty)), TemplateID = d.templateId, Date = DateTime.Parse(d.created).ToString() });
