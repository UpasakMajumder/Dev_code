    ...
    public partial class InventoryProductFilter : CMSAbstractBaseFilterControl
    {
        ...
        private void SetFilter()
        {
            ...
            if (!string.IsNullOrEmpty(txtProductSearch.Text))
            {
                ...
                where += $"SKUName like '% { searchText}%' or SKUDescription like '%{ searchText }%' or b.BrandName like '%{searchText }%' or s.SKUProductCustomerReferenceNumber like '%{ searchText }%' or QtyPerPack like '%{ searchText }%'";
            }
            if (where != null)
            {
