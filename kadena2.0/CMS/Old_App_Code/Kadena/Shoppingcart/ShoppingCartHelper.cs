...
namespace Kadena.Old_App_Code.Kadena.Shoppingcart
{
    public class ShoppingCartHelper
    {
        ...
        public static void UpdateRemainingBudget(OrderDTO orderDetails, int userID)
        {
            try
            {
                var campaign = DIContainer.Resolve<IKenticoCampaignsProvider>().GetCampaign(orderDetails.Campaign.ID);
                ...
                var fiscalYear = orderDetails.Type == OrderType.generalInventory ?
                                 ...
                                 orderDetails.Type == OrderType.prebuy ? (campaign?.GetValue("FiscalYear", string.Empty) ?? string.Empty) : string.Empty;
